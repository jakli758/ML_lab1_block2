...
The break criterion is checked beginning with second iteration, as there as a value to compare for the first time. It simply checks if there has been a significant change in the looklikihood compared to the last iteration.
```{r eval=FALSE}
if (it > 1){
...
print("Learned mu for 3 clusters:")
run_em(3)
print("True mu:")
true_mu
```
...
For too few clusters, in this case only, the tendency of the two mus still have similarity with some of the true mus (learned mu[1] is fairly close to true_mu[2], and learned mu[2] fairly close to true_mu[1], but it is already ~0.1 off. The reason is in the underfitting of the model, as it can't represent the data belonging to the missing clusters. 

```{r}
print("Learned mu for 2 clusters:")
run_em(2)
print("True mu:")
true_mu
```
...
```{r}
print("Learned mu for 4 clusters:")
run_em(4)
print("True mu:")
true_mu
```
